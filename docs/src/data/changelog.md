...
- Fixed RocketSim Connect's LLDB support for Xcode 27, restoring automatic network monitoring setup.
...
- RocketSim's main window can now be opened from the status bar menu.

**Fixed:**

- Fixed the macOS Screen Recording permission dialog showing multiple times at launch when access was not yet determined. The system prompt now only appears after pressing the "Open System Preferences" button on the permission onboarding screen.
- Fixed the macOS Screen Recording permission dialog reappearing on the first launch after a previously granted permission was revoked or reset.
