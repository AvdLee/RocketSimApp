...
modifiedTime: 2026-09-09T15:00:00.000Z
...
## What's new in RocketSim 16.4.6?

RocketSim 16.4.6 adds [Browser Preview](/blog/ios-simulator-browser-preview/) as a beta. The `rocketsim preview` command opens a live, interactive Simulator in your browser, where you can navigate the app, inspect accessibility elements, leave visual feedback across screens, and copy a structured prompt for your coding agent.

Agent commands keep this preview available automatically and include its URL in the `rs/1` response context. This means your coding agent can open the Simulator in a user-visible IDE browser while it works, allowing you to follow the navigation and review the final state yourself.

The release also reduces background CPU usage from recent-build scans and accessibility refreshes during agent sessions. Compact `nav` and `act` snapshots omit irrelevant keyboard, off-canvas, and duplicate elements, while improved recovery errors help agents respond to stale Simulator services and unavailable accessibility instead of failing with a generic error.

Capture workflows improve as well. A new **Open editor directly after recording** setting skips the initial conversion, so you can trim the recording before exporting. RocketSim also adds real device bezels for the 11-inch and 13-inch iPad Pro (M5) and prevents physical-device preview windows from becoming too narrow.
...
If you already use RocketSim, install the [latest RocketSim version from the Mac App Store](https://apps.apple.com/app/apple-store/id1504940162?pt=117264678&ct=rocketsim-16-physical-devices-ai-agents&mt=8). We recommend connecting a physical device to try Recent Builds, then opening **RocketSim → Settings → CLI & Agent** to install the CLI and Agent Skill. Feel free to [open an issue on GitHub](https://github.com/AvdLee/RocketSimApp/issues) if you have a physical-device or agent workflow you would like to see next.
